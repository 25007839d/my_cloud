...
def validateDataFrame(to_validate, avro_schema, partition_by_trans_dt_flg):
    ...
    for field in localFieldList:
        ...
        if not field["isNullable"]:
            validated_df = validated_df.withColumn("quarantine_null_"+ field["name"], when((validated_df[field["name"]].isNull()) | (trim(validated_df[field["name"]].cast("string")) == ""),struct(lit(field["name"]).alias("col_name"),validated_df[field["name"]].cast('string').alias("col_value"),lit("column can not be null or blank").alias("quarantine_reason"))).otherwise(lit(None)))
        if typeFound is not None:
            ...
            validated_df = validated_df.withColumn("quarantine_cast_"+ field["name"], when((((validated_df[field["name"]].isNotNull()) & (trim(validated_df[field["name"]].cast("string"))!= "")) & ((validated_df[field["name"]+"_original"].isNull()) | (trim(validated_df[field["name"]+"_original"].cast("string")) == ""))) | (validated_df[field["name"]].isNull() & ((validated_df[field["name"]+"_original"].isNotNull()) & (trim(validated_df[field["name"]+"_original"].cast("string")) != ""))),struct(lit(field["name"]).alias("col_name"),validated_df[field["name"]+"_original"].cast('string').alias("col_value"),concat(lit("could not cast to "),lit(typeFound)).alias("quarantine_reason"))).otherwise(lit(None)))
